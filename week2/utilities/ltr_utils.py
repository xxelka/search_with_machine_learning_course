...
def create_rescore_ltr_query(user_query, query_obj, click_prior_query, ltr_model_name, ltr_store_name,
    ...
    # add on the rescore
    # DONE print("IMPLEMENT ME: create_rescore_ltr_query")
    query_obj["rescore"] = {
        ...
        "query": {
            ...
            "rescore_query_weight": rescore_query_weight  # Magic number, but let's say boost LTR matches
        }
    }
...
def create_feature_log_query(query, doc_ids, click_prior_query, featureset_name, ltr_store_name, size=200, terms_field="_id"):
    # DONE print("IMPLEMENT ME: create_feature_log_query")
    return {
        'size': size,
        'query': {
            'bool': {
